<template>
  <div>
    <l-map ref="myMap" class="mhy-map" style="width: 100vw; height: 100vh" :crs="crs" :zoom="zoom" :bounds="maxBounds" :max-bounds="maxBounds" :max-zoom="maxZoom" :min-zoom="minZoom" :center="center" :options="{ zoomControl: false, noWrap: true, continuousWorld: true }" @click="selectPoint(-1)" @ready="doSomethingOnReady()">
      <l-tile-layer :url="url" />
      <l-control-zoom position="bottomright" />
      <l-marker v-for="item in markers" :key="item.id" :lat-lng="item.oneLatLang" :icon="setIcon(item)" :opacity="item.isMark ? 0.3 : 1.0" @click="selectPoint(item.id)">
    ...
    <div class="map-layer-control" :class="openLayer ? 'map-layer-control--visible' : 'map-layer-control--hidden'">
      ...
      <ul v-show="selectPanel === 1" class="layer-control__panel">
        <li v-for="item in treeData.tree" :key="item.id" class="layer-control__group" :class="closedGroup.includes(item.id) ? 'layer-control__group--folded' : ''">
          <div class="layer-control__group-title" @click="closeGroup(item.id)">
            {{ item.name }}
          </div>
          <ul class="layer-control__list">
            <li v-for="jtem in item.children" :key="jtem.id" class="layer-control__item" :class="selectPointType.includes(jtem.id) ? 'visible' : ''" @click="selectPointKind(jtem.id)">
              {{ jtem.visible }}
    ...
    <!-- <div class="bbs-qr">
      <el-row style="width: 105px; padding-top: 10px"> 扫码加微信<br />=v=<br />诶嘿~<br /> </el-row>
    </div> -->
  </div>
</template>
 ...
 *
 * 功能概述：
 * - 基于 Leaflet 实现的游戏地图展示
 * - 支持标点标记、筛选和详情展示
 * - 左侧控制面板包含图层筛选和自定义标点功能
 * - 地图支持缩放、平移和点击交互
 *
 * 技术栈：
 * - Vue 2 + vue2-leaflet
...
export default {
  ...
  data() {
    return {
      ...
      // 当前选中标点的 ID，-1 表示未选中
      selectId: -1,
      // 当前选中的面板：1=观测者标点，2=我的标点
      selectPanel: 1,
      // 选中的标点类型 ID 列表，用于筛选地图标点
      selectPointType: [],
      // 折叠的分组 ID 列表，控制左侧面板分组展开/折叠
      closedGroup: [],
      // 当前选中的标签页 ID，对应地图区域切换
      selectTab: 0,
      // 左侧控制面板的展开/收起状态
      openLayer: true,
      // 地图缩放级别
      ...
      minZoom: 1,
      // 地图边界限制，防止拖动超出地图范围
      maxBounds: L.latLngBounds(L.latLng(0, 0), L.latLng(-128, 128)),
      // 坐标系类型，使用简单坐标系（非地理坐标系）
      crs: L.CRS.Simple,
      // 地图中心点坐标
  ...
  created() {
    const self = this;
    // 将 markPoint 方法挂载到 window 对象，供弹窗内 onclick 事件调用
  ...
  methods: {
    ...
    async test() {
      ...
      for (const point of markers) {
        // 将标点数据转换为地图标记所需格式
        point["label_name"] = label_list.find((item) => {
          return item.id === point["label_id"];
        }).name;
        point["icon"] = label_list.find((item) => {
          return item.id === point["label_id"];
        }).icon;
        point["isMark"] = false;
        // 坐标转换：游戏坐标 -> Leaflet 地图坐标（简单坐标系）
        // y轴取负并偏移，x轴偏移后除以缩放系数64
    ...
    async test2() {
      ...
      // 在标签列表开头添加"全地图"选项（默认选中）
      mapPageLabel.list.unshift({
        id: 0,
        name: "全地图",
        pc_icon_url:
          "data:image/png;base64,",
        pc_icon_url2:
          "data:image/png;base64,",
      });
      // 将类型为2的页面标签移到列表开头（作为默认选中的区域标签）
    ...
    doSomethingOnReady() {
      ...
      this.map.on("popupopen", (e) => {
        const point = this.markers.find((item) => {
    ...
    /**
     * 选择标点类型（筛选地图标点）
     * @param {number} id - 标点类型ID
     * 点击已选中的类型则取消，未选中则添加
     */
    selectPointKind(id) {
      if (this.selectPointType.includes(id)) {
        this.selectPointType.splice(
          this.selectPointType.findIndex((item) => item.id === id),
          1
        );
      } else {
      ...
      for (const type of this.selectPointType) {
        markers.push(
          ...point_list.filter((item) => {
            return item["label_id"] === type;
          })
        );
      }
      for (const point of markers) {
    ...
    closeGroup(id) {
      if (this.closedGroup.includes(id)) {
        this.closedGroup.splice(
          this.closedGroup.findIndex((item) => item.id === id),
          1
        );
      } else {
    ...
    /**
     * 标记/取消标记标点
     * @param {Object} point - 标点对象
     * @param {number} point.id - 标点ID
     * @param {boolean} point.isMark - 是否已标记
     */
    markPoint(point) {
      console.log(point);
    ...
    /**
     * 创建自定义图标
     * @param {Object} item - 标点对象
     * @returns {L.DivIcon} Leaflet自定义图标实例
     */
    setIcon(item) {
      const oneIcon = L.divIcon({
        html:
          `<div class="mhy-game-gis-icon">
                <div class="gis-icon" style="background-image: url(` +
          item.icon +
          `)"></div>
              <div/>`,
     ...
     * 设置弹窗内容
     * @param {Object} item - 标点对象
     * @param {boolean} query - 是否需要从接口获取详情，默认true
     */
    async setText(item, query = true) {
      ...
      const htmlStr =
        `<div class="map-popup">
            ...
            ${contentStr}
            <div class="map-popup__switch ${classStr}" onclick=markPoint(` +
     ...
     * @param {number} id - 子节点ID
     * @returns {string} 父节点名称
     */
    findPName(tree, id) {
      let pId = null;
