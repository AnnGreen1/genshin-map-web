<template>
  <div>
    <l-map ref="myMap" class="mhy-map" style="width: 100vw; height: 100vh;" :crs="crs" :zoom="zoom" :bounds="maxBounds"
      :max-bounds="maxBounds" :max-zoom="maxZoom" :min-zoom="minZoom" :center="center"
      :options="{ zoomControl: false, noWrap: true, continuousWorld: true }" @click="selectPoint(-1)"
      @ready="doSomethingOnReady()">
      <l-tile-layer :url="url" />
      <l-control-zoom position="bottomright" />
      <l-marker v-for="(item) in markers" :key="item.id" :lat-lng="item.oneLatLang" :icon="setIcon(item)"
        :opacity="item.isMark ? 0.3 : 1.0" @click="selectPoint(item.id)">
    ...
    <div class="map-layer-control" :class="openLayer ? 'map-layer-control--visible' : 'map-layer-control--hidden'">
      ...
      <ul v-show="selectPanel === 1" class="layer-control__panel">
        <li v-for="(item) in treeData.tree" :key="item.id" class="layer-control__group"
          ...
          <ul class="layer-control__list">
            <li v-for="(jtem) in item.children" :key="jtem.id" class="layer-control__item"
              :class="selectPointType.includes(jtem.id) ? 'visible' : ''" @click="selectPointKind(jtem.id)">
              {{ jtem.visible }}
